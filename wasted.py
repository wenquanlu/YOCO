...
class CountingViT(nn.Module):
    ...
    def forward(self, x, seq_len):
        ...
        #x = x.repeat(1, 1, seq_len, 1) # x: (patch_num, batch, seq_len, dim)
        ...
        for i in range(len(p)):
            repeated_sequences = torch.cat([x[j].expand(seq_len[i], -1) for j in range(batch_size)], dim=0)
            packed_sequence = pack_padded_sequence(repeated_sequences, seq_len.tolist(), batch_first=True, enforce_sorted=True)
            lstm_out.append(self.lstm(packed_sequence))
